...
export default function WhyUs() {
  return (
    <section className="py-[110px] px-[70px] bg-[#e8faf8] max-[960px]:py-[70px] max-[960px]:px-6">

      {/* Head */}
      <div className="flex items-end justify-between mb-14 max-[960px]:flex-col max-[960px]:items-start max-[960px]:gap-3">
        <div>
          <span className="text-[0.68rem] tracking-[0.2em] uppercase text-[#0f766e] mb-2.5 block font-semibold">
            Why Choose Us
          </span>

          <h2 className="font-serif text-[clamp(2rem,3.5vw,3.2rem)] font-light leading-[1.15] text-black">
            Shopping the Way
            <br />
            <em className="italic text-[#0f766e]">It Should Be</em>
          </h2>
        </div>

        <p className="text-[0.82rem] text-gray-500 max-w-[200px] text-right leading-[1.7] max-[960px]:text-left max-[960px]:max-w-full">
      ...
      <div className="grid grid-cols-3 gap-6 max-[960px]:grid-cols-1">
        {cards.map((c, i) => (
          <div
            key={i}
            className="bg-white rounded-[14px] px-9 py-11
          ...
          >
            ...
            <h3 className="font-serif text-[1.4rem] font-semibold mb-3.5 text-black">
              {c.title}
            </h3>

            <p className="text-[0.85rem] text-gray-600 leading-[1.8]">
              {c.body}
            </p>
          </div>
        ))}
